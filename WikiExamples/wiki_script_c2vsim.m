...
plot(polyout)
part_TR = ginput;
part_TR(:,3) = Ftop(part_TR(:,1),part_TR(:,2)) - 30;
%% Write particles
fid = fopen('CV_particlesTR.ich','w');
fprintf(fid, '# Test particle file in transient velocity field\n');
fprintf(fid, '# Transient tracking requires the location and the release time in the field\n');
fprintf(fid, '# Eid Sid X Y Z Time\n');
fprintf(fid, '#\n');
fprintf(fid, '%d %d %.3f %.3f %.3f %.2f\n', [ones(size(part_TR,1),1) (1:size(part_TR,1))' part_TR 8022*ones(size(part_TR,1),1)]');
fclose(fid);
%% read streamlines
S = readICHNOStraj('c2vsim_out01__ireal_0000_iter_0000_proc_0000.traj');
%% plot
